/*  =========================================================================
    zm_asset_selftest.c - run selftests
...
// Tests for draft public classes:
    { "zm_asset", zm_asset_test },
#endif // ZM_ASSET_BUILD_DRAFT_API
#ifdef ZM_ASSET_BUILD_DRAFT_API
    { "private_classes", zm_asset_private_selftest },
#endif // ZM_ASSET_BUILD_DRAFT_API
    {0, 0}          //  Sentinel
};
...
{
    test_item_t *item;
    printf ("Running zm-asset selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("zm_asset_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running zm-asset test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
